...
public class SignTools {
	...
	private static boolean checkSign(String certFilePath, String certFileName, String signStr, String sign)
			throws Exception {
		try {
			LogUtil.info("# 开始验证签名信息...");
			String keyFile = certFilePath + certFileName;
			LogUtil.info("平台公钥地址为： " + keyFile);
...
//				//jar中运行
//				certData = JarUtil.getbyteForJar(keyFile);
//			}else{
//				certData = FileUtils.readFileToByteArray(new File(keyFile));
//			}
			LogUtil.info("cert--"+keyFile);
			...
			LogUtil.info("待签名字符串为： " + signStr);
			LogUtil.info("第三方平台签名值为： " + sign);
