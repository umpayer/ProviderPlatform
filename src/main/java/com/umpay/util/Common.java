...
//	}
	...
	public static String runJsonPost(String url, JSONObject jsonObject, String encoding) {
		...
		try {
			...
			post.setEntity(s);
			LogUtil.info("post请求"+post);
			HttpResponse res = httpclient.execute(post);
			LogUtil.info(res.getStatusLine().getStatusCode());
	...
	public static String runJsonPost1(String url, JSONObject jsonObject, String encoding) {
		...
		try {
			...
			LogUtil.info("post请求:"+ post.getAllHeaders()+"实体"+post.getEntity());
			LogUtil.info("请求参数body："+jsonObject);
			HttpResponse res = httpclient.execute(post);
			LogUtil.info(res.getStatusLine().getStatusCode());
			if (res.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
				String result = EntityUtils.toString(res.getEntity());// 返回json格式：
